...
def optimize(boost_type, numGrid,run_num,dataloader_name,BDTPARAMS_file_name,cwd):
    ...
    if boost_type != "best" and boost_type != "trial": 
    ...
    elif boost_type == "trial":
        trial_num = np.random.randint(1000)
        factory.BookMethod(dataloader, TMVA.Types.kBDT, "trial_"+str(trial_num), numGrid)
    else:
    ...
    if boost_type == "trial":
        for bst in boost_type_list:
            if bst in numGrid:
                bst_type = bst

        BDTPARAMS[bst_type].append(["trial_"+str(trial_num),numGrid,0,0])
...
if __name__ == "__main__":
    ...
    if str(sys.argv[1]) == "best" or str(sys.argv[1]) == "trial":
